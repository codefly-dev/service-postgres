...
def main():
    ...
    try:
        ...
        command = ["go", "test", "-race", "-json", "./libs/go", "-run", "^TestConnectionProfile", "-count=1", "-timeout=2m"]
        checked = subprocess.run(command, cwd=repo, env=env, stdout=subprocess.PIPE, stderr=subprocess.STDOUT)
